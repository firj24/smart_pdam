...
unsigned char flowsensor = 35; // Sensor Input
...
void loop() {
    ...
    if (currentTime >= (cloopTime + 1000)) {
        ...
        FirebaseJson json;
        json.set("/nama", "Keran B");
        json.set("/rate1", l_minute);
        json.set("/volume1", vol);
        json.set("/volume_mL", vol * 1000);

        if (Firebase.RTDB.setJSON(&firebaseData, "waterflowData/waterflow2", &json)) { // Updated path to include waterflowData node
